object Versions {
    const val BUILD_GRADLE_VERSION = "8.0.2"

    const val ANDROID_COMPILE_SDK_VERSION = 34
    const val ANDROID_MIN_SDK_VERSION = 24
    const val ANDROID_TARGET_SDK_VERSION = 34

    const val ANDROID_VERSION_CODE = 1
    const val ANDROID_VERSION_NAME = "1.0.1"

    // Dependencies (Alphabet sorted)
    const val ACCOMPANIST_PERMISSIONS_VERSION = "0.30.1"
    const val ANDROID_COMMON_KTX_VERSION = "0.1.1"
    const val ANDROID_CRYPTO_VERSION = "1.0.0"
    const val ANDROIDX_CORE_KTX_VERSION = "1.10.1"
    const val ANDROIDX_DATASTORE_PREFERENCES_VERSION = "1.0.0"
    const val ANDROIDX_LIFECYCLE_VERSION = "2.6.1"
    const val ANDROIDX_CORE_SPLASH_SCREEN_VERSION = "1.0.1"

    const val CHUCKER_VERSION = "3.5.2"
    const val COMPOSE_BOM_VERSION = "2024.04.00"
    const val ACTIVITY_COMPOSE = "1.9.0"
    const val COMPOSE_COMPILER_VERSION = "1.4.6"
    const val COMPOSE_NAVIGATION_VERSION = "2.7.0-alpha01"

    const val HILT_VERSION = "2.50"
    const val HILT_NAVIGATION_COMPOSE_VERSION = "1.2.0"

    const val JAVAX_INJECT_VERSION = "1"

    const val KOTLIN_VERSION = "1.8.20"

    const val KOTLINX_COROUTINES_VERSION = "1.7.3"

    const val MOSHI_VERSION = "1.15.0"

    const val OKHTTP_VERSION = "4.11.0"

    const val RETROFIT_VERSION = "2.9.0"

    const val ROOM_DB_VERSION = "2.5.0"

    const val TIMBER_LOG_VERSION = "4.7.1"


    // Configuration
    const val DETEKT_VERSION = "1.21.0"

    // Testing libraries
    const val TEST_ANDROIDX_CORE_VERSION = "1.4.0"
    const val TEST_EXT_JUNIT = "1.1.5"
    const val TEST_JUNIT_VERSION = "4.13.2"
    const val TEST_KOTEST_VERSION = "5.6.2"
    const val TEST_MOCKK_VERSION = "1.12.3"
    const val TEST_ROBOLECTRIC_VERSION = "4.10.2"
    const val TEST_RULES_VERSION = "1.5.0"
    const val TEST_TURBINE_VERSION = "0.13.0"
}
